...
class TimeLimit(gym.Wrapper):
    ...
    def step(self, action):
        # print(f"Assert TimeLimit step: step={self._step}, done={self._done}")
        assert not self._done, f"Must reset environment. {self._done} steps taken in last episode."
        obs, reward, done, info = self.env.step(action)
        ...
        if self._step >= self._duration:
            ...
            if "discount" not in info:
                info["discount"] = np.array(1.0).astype(np.float32)
        # print(f"TimeLimit step: step={self._step}, self._done={self._done}, done={done}, state={obs}")
        return obs, reward, done, info
...
class UUID(gym.Wrapper):
    def __init__(self, env):
        ...
        self.env_idx = getattr(env, "env_idx", getattr(self.unwrapped, "env_idx", None))
